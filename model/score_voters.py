...
def load_people(conn) -> pd.DataFrame:
    ...
    sql = """
        SELECT
            ...
            h.acs_median_income::float AS acs_median_income,
            h.acs_pct_college::float   AS acs_pct_college,
            h.acs_pct_owner_occ::float AS acs_pct_owner_occ,
            h.acs_pct_hispanic::float  AS acs_pct_hispanic,
            h.acs_pct_black::float     AS acs_pct_black
        FROM people p
...
NUMERIC_TURNOUT = [
    ...
    "acs_median_income","acs_pct_college","acs_pct_owner_occ",
    "acs_pct_hispanic","acs_pct_black",
]
...
def write_scores(person_ids, turnout_probs, dem_lean_probs, rep_lean_probs, conn, dry_run):
    if dry_run:
...
def main():
    ...
    print(f"  rep_lean_prob mean={rep_lean_probs.mean():.3f}  p10={np.percentile(rep_lean_probs,10):.3f}  p90={np.percentile(rep_lean_probs,90):.3f}")

    # Save models so we never have to retrain just to change the write-back
    artifacts = ROOT / "model" / "artifacts"
    artifacts.mkdir(exist_ok=True)
    turnout_model.save_model(str(artifacts / "turnout.cbm"))
    party_model.save_model(str(artifacts / "party.cbm"))
    print(f"  Models saved to {artifacts}/")
    ...
    try:
        ...
        pass
    conn2 = connect()
    write_scores(df["person_id"].tolist(), turnout_probs, dem_lean_probs, rep_lean_probs, conn2, args.dry_run)
    conn2.close()
