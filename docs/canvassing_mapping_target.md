...
| 1 | Native Expo app shell (iOS and Android): Start Route, Next Door, door detail, outcome sheet, encrypted local store, outbox sync. Apple Developer Program **individual** enrollment up front — no D-U-N-S number, so it no longer needs weeks of lead time. A Google Play account stays a separate decision, taken when Android work actually starts. | 3–4 weeks |
