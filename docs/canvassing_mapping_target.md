...
| Typed address in the app | **DECIDED (#12):** the phone's built-in geocoder first (Expo `geocodeAsync`, which uses Apple's geocoder on iOS). If it fails or returns nothing, fall back to the US Census Geocoder called from the Bellwether server. The result is always shown as a draggable pin for the canvasser to confirm. **Not** the public nominatim.openstreetmap.org service (about 1 request/second, no production use). |
...
- **Native app — DECIDED (full rollout):** React Native / Expo for **iOS only this cycle**, with **encrypted SQLite**. Android is dropped from scope (decision #13, updated 2026-09-21); Expo keeps it available later without a rewrite. A web app was rejected because it can't reliably encrypt local storage or sync in the background on iOS. Distribution and release details are in §14a.
...
| 1 | Native Expo app shell (iOS): Start Route, Next Door, door detail, outcome sheet, encrypted local store, outbox sync. Apple Developer Program **individual** enrollment up front — no D-U-N-S number, so it no longer needs weeks of lead time. | 3–4 weeks |
...
|---|---|
| Surfaces | **Canvasser:** native Expo app for iOS. **Field director and campaign manager:** Bellwether web dashboard plus the AI ops copilot. |
...
| Publisher account | **UPDATED 2026-09-21:** Apple Developer Program **individual** account (sole developer), not an organization. No D-U-N-S number — Apple requires one only from companies and educational institutions, so the multi-week pre-enrollment wait is gone. **Accepted consequences.** (a) *Seller name:* Apple lists the account holder's personal legal name as the seller on App Store product pages; a registered trade name or DBA is available only to organization enrollments. The developer name is set when the first app record is created and can't be edited afterwards in App Store Connect, though Apple documents a reviewed request process for changing it. (b) *Review risk:* App Review guideline 5.1.1(ix) says apps "that require sensitive user information should be submitted by a legal entity that provides the services, and not by an individual developer." Political canvassing is **not** in Apple's enumerated list of regulated fields (banking, healthcare, gambling, legal cannabis, air travel, crypto exchanges) and the wording is advisory rather than categorical — but an app showing voters' names and addresses sits inside reviewer discretion. This is the rationale the spec previously used to require an organization account; it is now a **named, accepted risk**, to be pre-empted in the R-01 reviewer notes by stating the operator relationship explicitly. (c) *EU exposure:* Digital Services Act trader rules would publish an individual's address (a P.O. Box is accepted with supporting documentation), phone and email on EU product pages. Excluding the 27 EU territories from availability avoids that; the seller name still shows in every storefront. **Not a one-way door:** Apple documents a request to convert an individual membership to an organization, but it is discretionary, needs a real legal entity plus a D-U-N-S number, has no published timeline, and can block submissions while pending — plan it as a change window, not a settings toggle. |
...
| Distribution | Beta: TestFlight (iOS). Production: the App Store with invite-code login, or Apple **unlisted** distribution, so the app doesn't need to be discoverable. **On an individual account (2026-09-21):** canvassers are TestFlight **external** testers — up to 10,000 per app, the first build of each version needs Beta App Review, and builds expire 90 days after upload. *Internal* testers are capped by the individual-enrollment limit of 50 additional App Store Connect users, so they cover Tim's own devices, not the volunteer pool. Unlisted distribution carries no documented individual-vs-organization bar, but it is a case-by-case approval and every audience Apple gives as an example is organizational — treat it as **unconfirmed** until actually requested, and keep the public-listing-plus-login route as the fallback. Apple Business Manager custom apps and the Enterprise Program are not available on an individual enrollment. An unlisted app is still fully reviewed and installable by anyone holding the link, so in-app authorization does the real gating, not the unlisted status. |
...
| Device reality | Volunteer-owned phones, no MDM. Security relies on §11 controls: encrypted cache, idle wipe, remote wipe flag, one active device per account. Support the last 2–3 iOS major versions. |
...
| 13 | App platform | **UPDATED 2026-09-21:** native Expo, **iOS only** with encrypted SQLite, full volunteer rollout. Android is dropped from this cycle's scope — one platform to build, test, review and support, with a single store relationship. Expo keeps Android reachable later without a rewrite. | Revisit only if the volunteer pool turns out to be materially Android-heavy. |
